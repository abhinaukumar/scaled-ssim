...
do
    ...
    ffmpeg -i "${directory}/${filename}.mp4" -vf scale=$resolution -sws_flags lanczos -y "temp/${resolution##*:}_scaled_video.mp4"
    ffmpeg -i "temp/${resolution##*:}_scaled_video.mp4" -vf scale=1920:1080 -sws_flags lanczos -y "temp/${resolution##*:}_upscaled_video.mp4"

    ffmpeg -i "temp/${resolution##*:}_upscaled_video.mp4" -i "${directory}/${filename}.mp4" -lavfi ssim="ssims/${filename}_${resolution##*:}_ref_ssim.log" -f null -
