...
if args.mode == 'analyze_training_size':
    ...
    for i in range(len(n_trains)):
        ...
        if args.n_feats == 2:
            ...
            train_feats = np.vstack([scale_data[:n_trains[i],:,:].flatten(), comp_data[:n_trains[i],:,:].flatten(), np.tile(np.expand_dims(1080/scales,-1),[n_trains[i], 1, n_qps]).flatten(), np.tile(51/qps,[n_trains[i], n_scales, 1]).flatten()]).T
            test_feats = np.vstack([scale_data[n_trains[i]:,:,:].flatten(), comp_data[n_trains[i]:,:,:].flatten(), np.tile(np.expand_dims(1080/scales,-1),[n_frames - n_trains[i], 1, n_qps]).flatten(), np.tile(51/qps,[n_frames - n_trains[i], n_scales, 1]).flatten()]).T
    ...
    if args.n_feats == 2:
        ...
        train_feats = np.vstack([scale_data[:n_train,:,:].flatten(), comp_data[:n_train,:,:].flatten(), np.tile(np.expand_dims(1080/scales,-1),[n_train, 1, n_qps]).flatten(), np.tile(51/qps,[n_train, n_scales, 1]).flatten()]).T
        test_feats = np.vstack([scale_data[n_train:,:,:].flatten(), comp_data[n_train:,:,:].flatten(), np.tile(np.expand_dims(1080/scales,-1),[n_frames - n_train, 1, n_qps]).flatten(), np.tile(51/qps,[n_frames - n_train, n_scales, 1]).flatten()]).T
